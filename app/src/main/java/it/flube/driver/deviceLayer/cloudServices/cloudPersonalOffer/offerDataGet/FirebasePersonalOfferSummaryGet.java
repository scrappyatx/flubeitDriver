 ...
 * Project : Driver
 */
public class FirebasePersonalOfferSummaryGet implements
        ValueEventListener {

    private static final String TAG = "FirebasePersonalOfferSummaryGet";

    private static final String BATCH_SUMMARY = "batch";

    private CloudPersonalOfferInterface.GetBatchSummaryResponse response;

    public void getBatchSummary(DatabaseReference batchDataNodeRef, String batchGuid, CloudPersonalOfferInterface.GetBatchSummaryResponse response){
    ...
    public void onDataChange(DataSnapshot dataSnapshot){
        ...
        if (dataSnapshot.exists()) {
            ...
            try {
                ...
                response.cloudGetPersonalOfferBatchSummarySuccess(batch);
            } catch (Exception e) {
                Timber.tag(TAG).w("      ...ERROR");
                Timber.tag(TAG).e(e);
                response.cloudGetPersonalOfferBatchSummaryFailure();
            }
        } else {
