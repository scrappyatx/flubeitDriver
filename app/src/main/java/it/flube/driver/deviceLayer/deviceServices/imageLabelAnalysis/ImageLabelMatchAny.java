 ...
 * Project : Driver
 */
public class ImageLabelMatchAny {
    private static final String TAG = "ImageLabelMatchAny";
    ...
    public void imageLabelMatchAnyRequest(ImageDetectionResults detected, ImageMatchSettings settings, Response response) {
        //// need to match ANY label in detected with a label in settings
        ...
        if (detected.getLabelMap().size() > 0) {
                ...
                if (label.getConfidence() >= settings.getConfidenceThreshold()) {
                    ...
                    for (String checkEntityId : settings.getMatchEntityIds()) {
                        ...
                        if (label.getEntityId().equals(checkEntityId)) {
                            ...
                            //// since we found a SINGLE match, the "foundMatchLabels" is TRUE
            ...
            response.imageLabelMatchAnyComplete(results);
    ...
    public interface Response {
        void imageLabelMatchAnyComplete(ImageMatchResults imageMatchResults);
    }
}
