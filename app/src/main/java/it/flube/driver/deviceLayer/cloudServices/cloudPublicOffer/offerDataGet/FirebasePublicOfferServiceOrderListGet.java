 ...
 * Project : Driver
 */
public class FirebasePublicOfferServiceOrderListGet implements ValueEventListener {
    private static final String TAG = "FirebasePublicOfferServiceOrderListGet";

    private static final String BATCH_DATA_SERVICE_ORDER_NODE = "serviceOrders";

    private CloudPublicOfferInterface.GetServiceOrderListResponse response;

    public void getServiceOrderListRequest(DatabaseReference batchDataRef, String batchGuid, CloudPublicOfferInterface.GetServiceOrderListResponse response){
    ...
    public void onDataChange(DataSnapshot dataSnapshot) {
        ...
        if (dataSnapshot.exists()) {
            ...
            try {
                ...
                response.cloudGetPublicOfferServiceOrderListSuccess(orderList);

            } catch (Exception e) {
                Timber.tag(TAG).w("   ...ERROR");
                Timber.tag(TAG).e(e);
                response.cloudGetPublicOfferServiceOrderListFailure();
            }
