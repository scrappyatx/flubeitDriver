 ...
 * Project : Driver
 */
public class FirebaseScheduledBatchOrderStepListGet implements ValueEventListener {
    private static final String TAG = "FirebaseScheduledBatchOrderStepListGet";
    private static final String BATCH_DATA_STEPS_NODE = "steps";
    private static final String SERVICE_ORDER_GUID = "serviceOrderGuid";

    private CloudScheduledBatchInterface.GetOrderStepListResponse response;

    public void getOrderStepList(DatabaseReference batchDataRef,
                                 String batchGuid, String serviceOrderGuid, CloudScheduledBatchInterface.GetOrderStepListResponse response){
    ...
    public void onDataChange(DataSnapshot dataSnapshot) {
        ...
        if (dataSnapshot.exists()) {
            ...
            try {
                ...
                response.cloudGetScheduledBatchOrderStepListSuccess(orderStepList);

            } catch (Exception e) {
                Timber.tag(TAG).w("   ...error while trying to get step data!");
                Timber.tag(TAG).e(e);
                response.cloudGetScheduledBatchOrderStepListFailure();
            }
        } else {
