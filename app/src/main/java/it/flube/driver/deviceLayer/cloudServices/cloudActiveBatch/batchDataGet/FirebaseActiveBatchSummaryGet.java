 ...
 * Project : Driver
 */
public class FirebaseActiveBatchSummaryGet implements
        ValueEventListener {

    private static final String TAG = "FirebaseActiveBatchSummaryGet";
    private static final String BATCH_SUMMARY = "batch";

    private CloudActiveBatchInterface.GetBatchSummaryResponse response;

    public void getBatchSummary(DatabaseReference batchDataNodeRef, String batchGuid, CloudActiveBatchInterface.GetBatchSummaryResponse response){
    ...
    public void onDataChange(DataSnapshot dataSnapshot){
        ...
        if (dataSnapshot.exists()) {
            ...
            try {
                ...
                response.cloudGetActiveBatchSummarySuccess(batch);
            } catch (Exception e) {
                Timber.tag(TAG).w("      ...ERROR");
                Timber.tag(TAG).e(e);
                response.cloudGetActiveBatchSummaryFailure();
            }
        } else {
